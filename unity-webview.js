var unityWebView = {
  ...
  init: function (name) {
    ...
    var $clone = $last
      ...
      .css("top", clonedTop + "%");
    var $iframe = $(
      '<iframe style="position:relative; width:100%; height100%; border-style:none; display:none; pointer-events:auto;" allow="camera https://villageharmony.fun:9001; microphone https://villageharmony.fun:9001"></iframe>'
    )
  ...
  setVisibility: function (name, visible) {
    if (visible) this.iframe(name).show();
    else this.iframe(name).hide();
  },
